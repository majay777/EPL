...
select any_value(distinct(t1.name)) as club, sum(m.team_h_score) as goals_conceded,case when month(any_value(m.kickoff_time)) in (8, 
            ...
            m  
            join  t1 on m.team_a = t1.id group by m.team_a order by goals_conceded desc 