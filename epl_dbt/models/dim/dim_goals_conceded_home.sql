...
select any_value(distinct(t1.name)) as CLUB, sum(m.team_a_score) as goals_conceded, case when month(any_value(m.kickoff_time)) in (8, 
            ...
            m
            join  t1 on m.team_h = t1.id group by m.team_h order by goals_conceded desc 