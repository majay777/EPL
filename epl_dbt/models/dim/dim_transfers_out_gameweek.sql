...
from (select distinct(e.web_name)      as                                                      NAME,
                     ...
                     (Select event from src_matches where finished = 'true' order by code desc limit 1) as gameweek,
     rank() over (partition by et.pos_name_short, e.web_name order by e.transfers_out_event desc) as RNK,
...
on e.element_type = et.id join t
    on e.team = t.id
order by (e.transfers_out_event) desc
    )
where rnk = 1
