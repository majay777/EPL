with e as (select *
   ...
   , el as (
select distinct (id) as id, web_name, second_name
...
select distinct(e.id)                   as id,
               ...
               (select max(Season) as Season from (select e.deadline_time, case when month (e.deadline_time) in (8,
                                                                                                                 9, 10,
                                                                                                                 12, 11) then year(e.deadline_time):: varchar || '-' || (year(e.deadline_time) + 1):: varchar when
                month(e.deadline_time) in (1,2,3,4,5) then 
                (year(e.deadline_time) - 1):: varchar || '-' ||  year(e.deadline_time):: varchar
end as Season from  m)) as Season
            from 
