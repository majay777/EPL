...
e as (
    select * from {{ ref("dim_events")  }}  where finished=True or is_current=True  QUALIFY row_number() over (partition by sid order by sid) = 1
)
...
select  chip_name, num_played, e.id, e.name,
               (select max(Season) as Season  from (select e.deadline_time, case when month(e.deadline_time) in (8, 
                ...
                from   c
                right join  e on c._dlt_parent_id = e._dlt_id where e.finished = True or is_current=True order by id