...
select POS, NAME, CLUB, transfers_in_event as transfers_in, Gameweek, Season
from (select distinct(e.web_name)      as                                                              NAME,
                     ...
                     (Select m.event
                      ...
                      order by m.code
                          desc                                                                         limit 1) as gameweek,
     rank() over (partition by et.pos_name_short, e.web_name order by e.transfers_in_event desc) as RNK,
...
from
    e join
    et
on e.element_type = et.id join t
    on e.team = t.id
order by (e.transfers_in_event) desc
    )
where rnk = 1