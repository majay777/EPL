...
select POS, NAME, CLUB, transfers_in_event as transfers_in, Gameweek, Season
            ...
            e.transfers_in_event,(Select  m.event  from src_matches m where m.finished = 'true' order by m.code desc limit 1) as gameweek,
            rank() over ( partition by POS, NAME order by e.transfers_in_event desc)  as RNK,
            ...
            on e.team = t.id order by (e.transfers_in_event) desc 
            ) where rnk = 1