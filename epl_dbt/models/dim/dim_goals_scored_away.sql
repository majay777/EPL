...
select any_value(distinct(t1.name)) as club, sum(m.team_a_score) as goals_scored, case when month(any_value(m.kickoff_time)) in (8, 
            ...
            (year(any_value(m.kickoff_time)) - 1):: varchar || '-' ||  year(any_value(m.kickoff_time)):: varchar
                ...
                m
                join  t1 on m.team_a = t1.id group by m.team_a order by goals_scored desc