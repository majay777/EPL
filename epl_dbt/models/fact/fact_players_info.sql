...
(select  distinct(t.id) as opponent_team_id, {{ dbt_utils.generate_surrogate_key(['web_name', 'second_name', 't.id', 'ev.id']) }} as player_id,ev.id as event_id, el.web_name,el.second_name, t1.id as club_team_id,
          ...
          union (select distinct(t.id) as opponent_team_id,{{ dbt_utils.generate_surrogate_key(['web_name', 'second_name', 't.id', 'ev.id']) }} as player_id, ev.id as event_id,
          el.web_name,el.second_name, t1.id as club_team_id,case    when m.team_a_score - m.team_h_score = 0 then 'D'
